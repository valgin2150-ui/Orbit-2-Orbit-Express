...
import { Newspaper, ExternalLink, Clock, ChevronRight, Lock } from "lucide-react";
...
export default function PayloadNewsFeed() {
  ...
  return (
    <Card className="bg-white border-gray-200 shadow-sm" data-testid="payload-news-card">
      <CardHeader className="pb-3 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg font-semibold text-gray-900 flex items-center gap-2">
            <Newspaper className="w-5 h-5 text-rail-red" />
            <span>Space Industry News</span>
          </CardTitle>
          <a 
            href="https://payloadspace.com" 
            target="_blank" 
            rel="noopener noreferrer"
            className="text-xs text-gray-500 hover:text-rail-red transition-colors flex items-center gap-1"
            data-testid="payload-link"
          >
            via Payload
      ...
      <CardContent className="pt-4 space-y-3">
        {isLoading ? (
          ...
          data.articles.slice(0, maxArticles).map((article) => (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex gap-3 p-2 hover:bg-gray-50 transition-all group"
              ...
              {article.imageUrl ? (
                <div className="w-16 h-16 overflow-hidden flex-shrink-0 bg-gray-100">
                  <img 
                    ...
                    alt={`Payload news: ${article.title || 'space payload article thumbnail'}`}
        ...
        <a 
          ...
          className="block"
        >
          <Button variant="outline" className="w-full border-gray-300 text-gray-600 hover:bg-gray-50 hover:text-gray-900 hover:border-rail-red/50 transition-all" data-testid="read-more-news-button">
            Read More on Payload
